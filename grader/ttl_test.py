...
def ttl_test ():
    ...
    try:
        ...
        # raw_send(src_ip, dst_ip, ttl=random.randint(1, ttl_conf['MinTTL']))
        # time.sleep(0.005)
        # raw_send(src_ip, sport, dst_ip, dport, [b'TTLTEST'], [0.0], ttl=random.randint(ttl_conf['MaxTTL'], 255))
        # time.sleep(0.005)
        ...
        for idx, (pkt, _) in enumerate(recv_log):
            ...
            try:
                ...
                print(f"[ttl] skipping truncated packet len={len(raw_pkt)} err={e}")
                continue
